...
class Bot {
  constructor(options) {
    ...
    // sticker: the sticker description if available, otherwise its name
    ...
    this.formatSticker = this.format.sticker || '<{$displayUsername}> [Sticker: {$sticker}]';
    ...
    this.channelUsers = {};
    this.pendingDiscordMessageStickers = new Map();
    this.pendingDiscordMessageReplyAuthors = new Map();
  ...
  attachListeners() {
    ...
    this.discord.on('raw', (packet) => {
      if (!packet || packet.t !== 'MESSAGE_CREATE' || !packet.d || !packet.d.id) return;

      // discord.js 12 drops sticker data from Message objects, so keep it from the gateway payload.
      const stickers = Bot.getRawMessageStickers(packet.d);
      if (stickers.length) {
        this.pendingDiscordMessageStickers.set(packet.d.id, stickers);
      }

      const replyAuthor = Bot.getRawReferencedMessageAuthor(packet.d);
      if (replyAuthor) {
        this.pendingDiscordMessageReplyAuthors.set(packet.d.id, replyAuthor);
      }
    });

    this.discord.on('message', (message) => {
      const stickers = this.pendingDiscordMessageStickers.get(message.id);
      const replyAuthor = this.pendingDiscordMessageReplyAuthors.get(message.id);
      let messageWithExtras = stickers && !Bot.getMessageStickers(message).length
        ? { ...message, stickerItems: stickers }
        : message;
      if (replyAuthor && !Bot.getReferencedMessage(messageWithExtras)) {
        messageWithExtras = { ...messageWithExtras, replyAuthor };
      }
      if (stickers) this.pendingDiscordMessageStickers.delete(message.id);
      if (replyAuthor) this.pendingDiscordMessageReplyAuthors.delete(message.id);

      // Ignore bot messages and people leaving/joining
      this.sendToIRCWithFetches(messageWithExtras);
    });
  ...
  static collectionValues(collection) {
    ...
    if (collection.cache) return Bot.collectionValues(collection.cache);
    ...
    if (typeof collection === 'object' && ('name' in collection || 'description' in collection)) {
      return [collection];
    }
    return [];
  }

  static getMessageStickers(message) {
    const stickers = Bot.collectionValues(message.stickers);
    if (stickers.length) return stickers;
    return Bot.collectionValues(message.stickerItems);
  }

  static getRawMessageStickers(data) {
    const stickerItems = Bot.collectionValues(data && data.sticker_items);
    if (stickerItems.length) return stickerItems;
    return Bot.collectionValues(data && data.stickers);
  }

  static getRawReferencedMessageAuthor(data) {
    if (!data || !data.referenced_message || !data.referenced_message.author) return null;

    return {
      author: data.referenced_message.author,
      member: data.referenced_message.member,
      content: data.referenced_message.content
    };
  }

  static getReferencedMessage(message) {
    if (message.referencedMessage && message.referencedMessage.author) {
      return message.referencedMessage;
    }

    if (message.replyAuthor && message.replyAuthor.author) {
      return message.replyAuthor;
    }

    const messageId = message.reference && message.reference.messageID;
    const messages = message.channel && message.channel.messages;
    if (messageId && messages && messages.cache && messages.cache.get) {
      return messages.cache.get(messageId);
    }

    return null;
  }

  static getDiscordAuthorName(author, guild, member) {
    if (member && (member.nickname || member.nick)) {
      return member.nickname || member.nick;
    }

    return Bot.getDiscordNicknameOnServer(author, guild);
  }

  static formatToRegex(format, captureKey) {
    let lastIndex = 0;
    const parts = [];
    const matchPattern = /{\$(.+?)}/g;
    let match = matchPattern.exec(format);

    while (match) {
      parts.push(_.escapeRegExp(format.slice(lastIndex, match.index)));
      parts.push(match[1] === captureKey ? '(.+?)' : '[\\s\\S]*?');
      lastIndex = match.index + match[0].length;
      match = matchPattern.exec(format);
    }

    parts.push(_.escapeRegExp(format.slice(lastIndex)));
    return new RegExp(`^${parts.join('')}$`);
  }

  static getAuthorFromFormattedDiscordMessage(content, format) {
    if (!content) return null;

    const authorKeys = ['author', 'nickname', 'displayUsername'];
    const matchingAuthorKey = authorKeys.find(key => format.includes(`{$${key}}`));
    if (!matchingAuthorKey) return null;

    const match = content.match(Bot.formatToRegex(format, matchingAuthorKey));
    return match && match[1] ? match[1] : null;
  }

  getDiscordReplyPrefix(message) {
    const referencedMessage = Bot.getReferencedMessage(message);
    if (!referencedMessage || !referencedMessage.author) return '';

    const bridgedAuthor = referencedMessage.author.id === this.discord.user.id
      ? Bot.getAuthorFromFormattedDiscordMessage(referencedMessage.content, this.formatDiscord)
      : null;
    const authorName = Bot.getDiscordAuthorName(
      referencedMessage.author,
      message.guild,
      referencedMessage.member
    );

    return `${bridgedAuthor || authorName}: `;
  }

  static getStickerText(sticker) {
    if (!sticker) return null;
    const description = typeof sticker.description === 'string' ? sticker.description.trim() : '';
  ...
  async sendToIRCWithFetches(message) {
    if (Bot.getReferencedMessage(message)) {
    ...
    const messageId = message.reference && message.reference.messageID;
    const messages = message.channel && message.channel.messages;
    if (!messageId || !messages || !messages.fetch) {
      this.sendToIRC(message);
      return;
    }

    try {
      const referencedMessage = await messages.fetch(messageId);
      this.sendToIRC(referencedMessage && referencedMessage.author
        ? { ...message, replyAuthor: referencedMessage }
        : message);
    } catch (error) {
      logger.debug('Unable to fetch referenced Discord message', messageId, error);
      this.sendToIRC(message);
    }
  }
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      const replyPrefix = this.getDiscordReplyPrefix(message);
      ...
      if (this.isCommandMessage(text)) {
        ...
        this.ircClient.say(ircChannel, `${replyPrefix}${text}`);
      } else {
        if (text !== '') {
          // Convert formatting
          text = formatFromDiscordToIRC(text);
          patternMap.text = `${replyPrefix}${text}`;
        ...
        Bot.getMessageStickers(message).forEach((sticker) => {
          const stickerText = Bot.getStickerText(sticker);
          if (!stickerText) return;

          const stickerMessage = Bot.substitutePattern(this.formatSticker, {
            ...patternMap,
            sticker: stickerText
          });

          logger.debug('Sending sticker to IRC', ircChannel, stickerMessage);
          this.ircClient.say(ircChannel, stickerMessage);
        });
